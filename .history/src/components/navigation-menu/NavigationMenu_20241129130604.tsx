...
const NavigationMenu: React.FC = () => {
  ...
  // Handler for submenu link clicks
  const handleSubmenuLinkClick = (isActive: boolean) => (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
    if (isActive) {
      setIsAnimating(true);
      setIsSubmenuVisible(false);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
      event.preventDefault(); // Prevent navigation if not active
    }
  };
      ...
      <nav className={styles.navigationBar}>
        <div className={styles.navLogo}>
          <Link to="/" className={styles.logo}>
            <img title="Home Page" src={pageLogo} alt="logo" />
          </Link>
        </div>
                ...
                <div className={styles.submenuSection} key={key}>
                  ...
                  {submenu[key].map((item, index) => (
                    <Link 
                      ...
                      onClick={handleSubmenuLinkClick(item.isActive)}
                      style={{
                        pointerEvents: item.isActive ? 'auto' : 'none', // Disable pointer events if not active
                        textDecoration: 'none',
                      }}
                    >
                      <p
